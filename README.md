...
- **Supervision Portainer** : https://localhost:9443
________________________________________

## 🛠️ Infrastructure & Supervision
L'architecture repose sur trois piliers complémentaires pour garantir une gestion efficace de la plateforme de vote :

1. **MySQL 8.0** : Serveur de base de données relationnelles stockant l'ensemble des données de vote. La persistance est assurée par des volumes Docker.
2. **phpMyAdmin** : Interface d'administration Web permettant de gérer les structures de tables, d'exécuter des requêtes SQL et d'importer/exporter les données facilement.
3. **Portainer** : Solution de supervision (Monitoring) pour surveiller en temps réel l'état des conteneurs, l'utilisation du processeur (CPU) et de la mémoire vive (RAM).
